/**
 * Pure JavaScript SHA-256 fallback when Web Crypto API is not available
 * (e.g. non-secure contexts like http://192.168.x.x)
 */
...
/**
 * Hashes a password string using SHA-256 via Web Crypto API with fallback to Pure JS.
...
export async function hashPassword(password: string): Promise<string> {
  try {
    ...
    console.warn('[Crypto] Web Crypto subtle unavailable or failed, using fallback:', e);
  }
